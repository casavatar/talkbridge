...
class PromptEngineer:
    ...
    def test_prompt(self, prompt: str, model: Optional[str] = None,
        ...
        try:
            # Ensure we get a string response, not a generator
            response = self.client.generate(model, prompt, stream=False)
            end_time = time.time()
            ...
            if isinstance(response, str):
                ...
                # In case response is None or other type
            ...
            if expected_response and response_str:
                # Simple similarity metric
                similarity = self._calculate_similarity(response_str, expected_response)
    ...
    def analyze_prompt_performance(self, template_name: str) -> Dict[str, Any]:
        ...
        # Calculate metrics
        response_times = [r.performance_metrics.get('response_time', 0) for r in template_results if r.performance_metrics]
        response_lengths = [r.performance_metrics.get('response_length', 0) for r in template_results if r.performance_metrics]
