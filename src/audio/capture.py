...
from ..utils.error_handler import handle_error, retry_with_backoff, RetryableError, CriticalError

logger = get_logger(__name__)

def _safe_device_attr(device_info: Any, key: str, default: Any = None) -> Any:
    """Safely get device attribute from sounddevice device info."""
    ...
    try:
        if hasattr(device_info, '__getitem__'):
            return device_info[key]
    except (KeyError, TypeError, IndexError):
        pass
    return default

def _safe_device_access(devices: Any, index: int) -> Any:
    """Safely access device from device list."""
    if devices is None:
        return {}
    try:
        if hasattr(devices, '__getitem__') and isinstance(index, int):
...
class AudioCapture:
    def __init__(self, sample_rate=None, channels=1, device=None, loopback_mode=False):
        ...
        try:
            if device is not None:
                ...
                device_name = _safe_device_attr(device_info, 'name', 'Unknown')
                inputs = _safe_device_attr(device_info, 'max_input_channels', 0)
                outputs = _safe_device_attr(device_info, 'max_output_channels', 0)
                default_sr = _safe_device_attr(device_info, 'default_samplerate', 'Unknown')
                logger.info(f"Selected device {device}: {device_name} "
                          f"(inputs: {inputs}, outputs: {outputs}, default_sr: {default_sr})")
            else:
                default_device = sd.query_devices(kind='input')
                device_name = _safe_device_attr(default_device, 'name', 'Unknown')
                inputs = _safe_device_attr(default_device, 'max_input_channels', 0)
                logger.info(f"Using default input device: {device_name} (inputs: {inputs})")
        ...
        try:
            ...
            try:
                ...
                logger.debug(f"Default input device: {_safe_device_attr(input_device, 'name', 'Unknown')}")
                logger.debug(f"Default output device: {_safe_device_attr(output_device, 'name', 'Unknown')}")
            except Exception as e:
    ...
    def initialize_device_by_index(self, device_index: int | str | None = "auto", samplerate: int = 44100, channels: int = 1) -> bool:
        ...
        try:
            ...
            if device_index == "auto" or device_index is None:
                ...
                # Device name lookup
                matches = [i for i, d in enumerate(devices) if device_index.lower() in _safe_device_attr(d, "name", "").lower()]
                if not matches:
                    logger.error(f"No device found matching: {device_index}")
                    return False
                selected_index = matches[0]
                device_info = _safe_device_access(devices, selected_index)
                logger.info(f"Found device match: {_safe_device_attr(device_info, 'name', 'Unknown')}")
            else:
                logger.error(f"Invalid device_index type: {type(device_index)}")
                return False
            
            # Validate the selected device has input channels
            device_info = _safe_device_access(devices, selected_index) if isinstance(selected_index, int) else {}
            max_input_channels = _safe_device_attr(device_info, 'max_input_channels', 0)
            if max_input_channels <= 0:
                device_name = _safe_device_attr(device_info, 'name', 'Unknown')
                logger.error(f"Device {selected_index} ({device_name}) has no input channels")
                return False
            ...
            # Set sounddevice defaults - use try/except for type safety
            try:
                if isinstance(selected_index, int):
                    sd.default.device = selected_index  # type: ignore
                sd.default.samplerate = self.sample_rate  # type: ignore
                sd.default.channels = self.channels  # type: ignore
            except (AttributeError, TypeError) as e:
                logger.warning(f"Could not set sounddevice defaults: {e}")
        ...
        try:
            ...
            if system == "windows":
                ...
                for i, device_info in enumerate(devices):
                    device_name = _safe_device_attr(device_info, 'name', '').lower()
                    
                    # Look for output devices that support loopback
                    max_output_channels = _safe_device_attr(device_info, 'max_output_channels', 0)
                    if isinstance(max_output_channels, (int, float)) and max_output_channels > 0:
                        ...
                        if any(keyword in device_name for keyword in [
                            ...
                            loopback_devices.append({
                                'index': i,
                                'name': _safe_device_attr(device_info, 'name', 'Unknown'),
                                'platform': 'windows',
                                'type': 'wasapi_loopback',
                                'channels': max_output_channels,
                                'sample_rate': _safe_device_attr(device_info, 'default_samplerate', 44100),
                                'hostapi': _safe_device_attr(device_info, 'hostapi', None)
                            })
                ...
                for i, device_info in enumerate(devices):
                    ...
                    if 'monitor' in device_name or 'loopback' in device_name:
                        loopback_devices.append({
                            ...
                            'platform': 'linux',
                            'type': 'alsa_monitor',
                            'channels': _safe_device_attr(device_info, 'max_input_channels', 2),
                            'sample_rate': _safe_device_attr(device_info, 'default_samplerate', 44100),
                            'hostapi': _safe_device_attr(device_info, 'hostapi', None)
                        })
    ...
    def is_loopback_supported(self) -> Tuple[bool, str]:
        ...
        if system == "windows":
            ...
            try:
                devices = sd.query_devices()
                wasapi_devices = [d for d in devices if _safe_device_attr(d, 'hostapi', -1) == 2]  # WASAPI
                if wasapi_devices:
    ...
    def record_system_audio_chunk(self, duration: float = 0.1, device_index: Optional[int] = None) -> Optional[np.ndarray]:
        ...
        try:
            ...
            if system == "windows":
                ...
                # Convert device_index to string for Linux
                device_name = str(device_index) if device_index is not None else None
                return self._record_linux_loopback(duration, device_name)
            elif system == "darwin":
        ...
        try:
            ...
            if device_index is None:
                ...
                for i, d in enumerate(devices):
                    hostapi = _safe_device_attr(d, 'hostapi', -1)
                    max_output = _safe_device_attr(d, 'max_output_channels', 0)
                    if hostapi == 2 and isinstance(max_output, (int, float)) and max_output > 0:
    ...
    def _get_optimal_sample_rate(self, device=None):
        ...
        try:
            ...
            device_info = sd.query_devices(device)
            default_rate = int(_safe_device_attr(device_info, 'default_samplerate', 44100))
    ...
    def _validate_and_get_supported_sample_rate(self, requested_rate):
        ...
        try:
            ...
            device_info = sd.query_devices(device)
            device_name = _safe_device_attr(device_info, 'name', 'Unknown')
            device_samplerate = _safe_device_attr(device_info, 'default_samplerate', 44100)
            logger.debug(f"Using device: {device_name}, default rate: {device_samplerate}")
            ...
            fallback_rates = [44100, 48000, 22050, 16000, 8000, int(device_samplerate)]
    ...
    def start_output_stream(self, callback: Optional[Callable] = None, device: Optional[int] = None):
        ...
        try:
                        ...
                        for line in lines:
                            if 'monitor' in line.lower():
                                ...
                                if len(parts) > 1:
                                    ...
                                    break
                except Exception:  # Broader catch for subprocess errors
                    pass
            ...
            if isinstance(loopback_device, str):
                ...
                device_info = _safe_device_access(devices, loopback_device)
                device_name = _safe_device_attr(device_info, 'name', 'Default')
            ...
            # Log device information before recording
                try:
                    if self.device is not None:
                        ...
                        device_name = _safe_device_attr(device_info, 'name', 'Unknown')
                        inputs = _safe_device_attr(device_info, 'max_input_channels', 0)
                        hostapi = _safe_device_attr(device_info, 'hostapi', 'Unknown')
                        logger.debug(f"Using device {self.device}: {device_name} "
                                   f"(inputs: {inputs}, hostapi: {hostapi})")
                    else:
                        default_device = sd.query_devices(kind='input')
                        device_name = _safe_device_attr(default_device, 'name', 'Unknown')
                        logger.debug(f"Using default input device: {device_name}")
                except Exception as e:
                    logger.warning(f"Could not query device info: {e}")            # Validate sample rate with device before recording
