    ...
    def _setup_animation(self):
        ...
        for particle in self.particles:
            circle = Circle((particle['x'], particle['y']), particle['size'], 
                          facecolor=viridis_cmap(particle['color']), alpha=0.7)
...
class GeometricAnimation:
    ...
    def _update_animation(self):
        ...
        # Apply transformations based on shape type
        if self.shape == "circle" and isinstance(self.shape_patch, Circle):
            self.shape_patch.center = (0.5 * np.sin(self.angle), 0.3 * np.cos(self.angle))
            self.shape_patch.radius = 0.5 * self.scale
        elif self.shape == "square" and isinstance(self.shape_patch, Rectangle):
        ...
        elif self.shape == "triangle" and isinstance(self.shape_patch, Polygon):
            ...
            # Convert to proper format for Polygon.set_xy()
            vertices = np.array(rotated_points)
            self.shape_patch.set_xy(vertices)
        elif self.shape == "polygon" and isinstance(self.shape_patch, Polygon):
