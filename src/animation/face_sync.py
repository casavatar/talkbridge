...
class FaceSync:
    ...
    def __init__(self, use_webcam: bool = True, avatar_path: Optional[str] = None):
        ...
        self.audio_sample_rate: Optional[float] = None
        ...
        if MEDIAPIPE_AVAILABLE and mp_face_mesh is not None:
            try:
                ...
                self.logger.warning(f"Failed to initialize MediaPipe FaceMesh: {e}")
                self.face_mesh = None
        ...
        if use_webcam:
            try:
                ...
                if not self.cap.isOpened():
                    ...
                    if not avatar_path:
                        default_avatar_path = str(Path(__file__).parent / "resources" / "default_avatar.png")
    ...
    def detect_facial_landmarks(self, frame: np.ndarray) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Check if results has face landmarks (support both older and newer MediaPipe versions)
    ...
    def get_frame(self) -> Optional[np.ndarray]:
        ...
        if self.use_webcam and self.cap is not None:
            ...
            if ret:
                return frame
        elif not self.use_webcam and hasattr(self, 'avatar_image') and self.avatar_image is not None:
