...
class TTSAdapter:
    ...
    def __init__(self, engine: str = "system", voice: Optional[str] = None):
        ...
        if not TTS_AVAILABLE:
            raise ImportError("TTSSynthesizer module not available")
    ...
    def synthesize(self, text: str, language: str = "en", voice: Optional[str] = None) -> SynthesisResult:
        ...
        try:
            ...
            # Use TTSSynthesizer's synthesize method
            result = self.tts_synthesizer.synthesize(
                text=text,
                language=language,
    ...
    def get_voices(self, language: Optional[str] = None) -> List[str]:
        ...
        try:
            if hasattr(self.tts_synthesizer, 'get_voices'):
                voices = self.tts_synthesizer.get_voices(language=language)
                if isinstance(voices, list):
                    return voices
                elif isinstance(voices, dict):
                    # If voices are returned as dict, extract voice names/IDs
                    return list(voices.keys())
                else:
                    return []
            else:
                # Return some common voice names as fallback
