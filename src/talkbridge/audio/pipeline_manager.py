...
class PipelineManager:
    ...
    def __init__(self, 
        ...
        self.mic_capture: Optional[AudioCapture] = None
        self.sys_capture: Optional[AudioCapture] = None
        ...
        self.stt_adapter: Optional[WhisperSTTAdapter] = None
        self.translation_adapter: Optional[TranslationAdapter] = None
        ...
        self.last_activity_time = {
            AudioSourceType.MICROPHONE: 0,
            AudioSourceType.SYSTEM_AUDIO: 0
        }
    ...
    def start_mic_stream(self, 
        ...
        try:
            ...
            self._notify_audio_event(f"Microphone started: {device_name}", Level.INFO, "audio_system")
    ...
    def start_system_stream(self, 
        ...
        try:
            ...
            # Send user notification
            self._notify_audio_event(f"System audio started: {device_name}", Level.INFO, "audio_system")
    ...
    def set_target_language(self, language: str):
        ...
        self.logger.info(f"Target language set to: {language}")
    ...
    def handle_component_failure(self, component_name: str, error: Exception):
        ...
        if component_name == "stt_adapter":
            ...
            if self.async_runner and UI_INTEGRATION_AVAILABLE:
                ...
                self.async_runner.run_async(
                    ...
                    on_success=lambda: self.logger.info("Microphone successfully restarted"),
                    on_error=lambda e: self._handle_restart_failure("microphone", e)
                )
            else:
                # Fallback to blocking restart
                try:
                    self.stop_mic_stream()
                    threading.Timer(1.0, lambda: self.start_mic_stream(self.selected_mic_device)).start()
            ...
            if self.async_runner and UI_INTEGRATION_AVAILABLE:
                ...
                self.async_runner.run_async(
                    ...
                    on_success=lambda: self.logger.info("System audio successfully restarted"),
                    on_error=lambda e: self._handle_restart_failure("system_audio", e)
                )
            else:
                # Fallback to blocking restart
                try:
                    self.stop_system_stream()
                    threading.Timer(1.0, lambda: self.start_system_stream(self.selected_sys_device)).start()
                except Exception as restart_error:
        ...
        if UI_INTEGRATION_AVAILABLE:
            try:
                notify(
                    f"Audio component '{component_name}' encountered an error",
                    Level.WARNING,
    ...
    def _restart_microphone_async(self):
        ...
        if not result:
            raise UserFacingError(
                "Failed to restart microphone audio capture",
    ...
    def _restart_system_audio_async(self):
        ...
        if not result:
            raise UserFacingError(
                "Failed to restart system audio capture",
                ErrorCategory.AUDIO_ERROR,
    ...
    def _handle_restart_failure(self, component_type: str, error: Exception):
        ...
        if UI_INTEGRATION_AVAILABLE:
            try:
                notify(
                    f"Failed to restart {component_type} audio",
                    Level.ERROR,
                    category="audio_system"
                )
            except Exception as e:
    ...
    def _notify_audio_event(self, message: str, level: Level = Level.INFO, category: str = "audio_pipeline"):
        """Send audio event notification to UI if available."""
        if UI_INTEGRATION_AVAILABLE:
            try:
                notify(message, level, category=category)
            except Exception as e:
