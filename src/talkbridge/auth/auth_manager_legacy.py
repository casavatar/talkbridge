...
class AuthManager:
    ...
    def _load_users_cached(self) -> Dict[str, Dict]:
        ...
        if (AuthManager._users_cache is not None and 
            ...
            try:
                ...
                logger.debug(f"Failed to check user file modification time: {e}")
                # Continue to load users from file
        ...
        try:
            ...
            logger.debug(f"Failed to get modification time for cache: {e}")
    ...
    def _load_users(self) -> Dict[str, Dict]:
        ...
        if self.users_file.exists():
            try:
                ...
                # These are retryable file system errors
                raise RetryableError(f"Failed to load users file due to file system error: {e}") from e
            except (json.JSONDecodeError, ValueError) as e:
                # These are data corruption errors - not retryable
                handle_error(e, "Auth", f"Corrupted users file: {self.users_file}", logger)
                return self._create_default_users()
            except Exception as e:
                handle_error(e, "Auth", f"Failed to load users file: {self.users_file}", logger)
                return self._create_default_users()
        else:
    ...
    def _save_users(self, users: Dict[str, Dict]) -> None:
        ...
        try:
            ...
            # These are retryable file system errors
            raise RetryableError(f"Failed to save users due to file system error: {e}") from e
        except Exception as e:
            handle_error(e, "Auth", f"Failed to save users to {self.users_file}", logger)
            raise CriticalError(f"Unable to save user data: {e}") from e
