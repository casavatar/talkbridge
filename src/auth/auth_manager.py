        ...
        try:
            ...
            for user_data in dev_users:
                ...
                if not existing_user:
                    # Get password from environment - NO DEFAULTS
                    from .utils.password_config import PasswordConfig
                    password = PasswordConfig.get_password_from_env(user_data["username"])
                    if not password:
                        logger.error(f"No password found for dev user {user_data['username']} - skipping")
                        continue
